...
# Read in data ------------------------------------------------------------
  ...
  tar_target(
    ...
    haven::read_sas(consol_dat_fp)  |>
      dplyr::filter(Geolevel == 'County',
                        YearID == '2020'), # Don't care about state rihgt now
    format = 'parquet'
  ),
  ...
  tar_target(
    ...
    joinDataWithDictionary(consol_dat, data_dict) |> # reorder columns
      ...
      filter(RaceName == 'Total',
             ...
             EducationName == 'Total',
             EstimateName == 'Percentage'),
    format = 'parquet',

  ),

  tar_target(wider_consol_dat,
             pivot_wider(consol_dat_dict_combined,
  ...
  tar_target(diab_prev_binary_network,
             setupNetwork(adj_matrix_diab, adj_matrix_sci, complete_wider_consol_dat),
             deployment = 'main'),
  # Prevalence diabetes sum of diabetes as a continuous edge
  tar_target(prev_sum_network,
             setupNetwork(adj_matrix_prev_sum, adj_matrix_sci, complete_wider_consol_dat)),

  # Test the model where diabetes prevalence is binary and sci as edgecov
  tar_target(test_model,
             ergm(diab_prev_binary_network ~ edges + nodecov('obesity') + edgecov('log_scaled_sci'))),
